...
**The `.gitignore` patterns are stack-specific, and the list is a floor.** Credential-bearing files are named by the ecosystem, not by a universal convention — R's `.Renviron` does the job of `.env`, and nothing generic matches it. Adoption writes a universal block plus a block per stack it detects, but a checklist can only find what it already names: passing it means the known patterns are present, not that the project is covered. Look at what the repo actually holds before you call the step done.
